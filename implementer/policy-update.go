	...
	if op == policies.UpdateRemove {
		...
		for _, ingressACLEntry := range ingressACLEntryList {
			for _, staleACLEntry := range staleIngressACLEnteries {
				if staleACLEntry.NetworkID == ingressACLEntry.NetworkID &&
					staleACLEntry.LocationID == ingressACLEntry.LocationID &&
					staleACLEntry.NetworkType == ingressACLEntry.NetworkType &&
					staleACLEntry.LocationType == ingressACLEntry.LocationType &&
					staleACLEntry.Protocol == ingressACLEntry.Protocol &&
					staleACLEntry.SourcePort == ingressACLEntry.SourcePort &&
					staleACLEntry.DestinationPort == ingressACLEntry.DestinationPort {
		...
		for _, egressACLEntry := range egressACLEntryList {
			for _, staleACLEntry := range staleEgressACLEnteries {
				if staleACLEntry.NetworkID == egressACLEntry.NetworkID &&
					staleACLEntry.LocationID == egressACLEntry.LocationID &&
					staleACLEntry.NetworkType == egressACLEntry.NetworkType &&
					staleACLEntry.LocationType == egressACLEntry.LocationType &&
					staleACLEntry.Protocol == egressACLEntry.Protocol &&
					staleACLEntry.SourcePort == egressACLEntry.SourcePort &&
					staleACLEntry.DestinationPort == egressACLEntry.DestinationPort {
